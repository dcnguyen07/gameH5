...
async function init() {
    ...
    const firstOpen = storage.getStorageItem('firstopen') ?? false;
    if (!firstOpen) {
        const platform = getPlatformAndBrowser();
        FirebaseEventLogger.log('first_open', {
            platform: platform.platform,
            browser: platform.browser,
        });
        storage.setStorageItem('firstopen', true);
    }
