...
export class LevelSystem implements System {
                    ...
                    gsap.to(line, {
                        ...
                        onComplete: () => {
                            ...
                            if (index === this.lines.length - 1) {
                                cb();
                                this._newLineTween.resume();
                                pause.removeTween(tween);
    ...
    public removeLine(line: BubbleLine) {
        // Remove the line from the grid
        this.lines.splice(this.lines.indexOf(line), 1);
        line.removeAllBubbles();
        // Return the line to the pool
        pool.return(line);
    }
    ...
    public setBubble(bubble: Bubble, gridI: number, gridJ: number) {
        ...
        tween.then(() => {
            ...
            specials.forEach((s) => {
                this.game.systems.get(PowerSystem).usePower(s.type, s.i, s.j);
                if (s.type !== 'super') {
                    this.removeBubble(s, false);
                }
            });
