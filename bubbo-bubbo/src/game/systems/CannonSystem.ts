...
import { boardConfig, SpecialBubbleType } from '../boardConfig';
...
export class CannonSystem implements System {
    ...
    public init() {
        ...
        const hudSystem = this.game.systems.get(HudSystem);
        hudSystem.cannonContainer.addChild(this.view);

        hudSystem.specialItemPanel.signalSelected.connect((type) => {
            this.addSpecial(type);
        });
    ...
    public enable(enable: boolean) {
        this.game.hitContainer.interactive = enable;
    }
    ...
    private addSpecial(type: SpecialBubbleType) {
        this.currentBubbleType = type;
        this.cannon.type = type;
    }
}
