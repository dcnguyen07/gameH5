...
import { GameOverScreen } from '../screens/overlays/GameOverScreen';
...
export class Game {
     ...
     */
    public addToGame(...views: Container<any>[]) {
        views.forEach((view) => {
     ...
     */
    public removeFromGame(...views: Container<any>[]) {
        views.forEach((view) => {
    ...
    public async gameOver() {
        ...
        this._enableAimSystem(false);
        // Update the highscore
        this.systems.get(ScoreSystem).updateHighscore();
        this.isGameOver = true;
        navigation.showOverlay(GameOverScreen, {
            score: this.stats.get('score'),
            callback: this._gameOverCallback.bind(this),
        });
    }

    private _enableAimSystem(isEnable: boolean) {
        const aimSystem = this.systems.get(AimSystem);
        aimSystem.enabled(isEnable);

        const canonSystem = this.systems.get(CannonSystem);
        canonSystem.enable(isEnable);
    }

    private async _gameOverCallback(state: 'ads' | 'skip') {
        ...
        if (state === 'ads') {
            ...
            this.systems.get(LevelSystem).scrollGamePlayUp(() => {
                this.isDied = false;
                this._enableAimSystem(true);
                this.systems.get(HudSystem).specialItemPanel.checkIsRewardAdsAvailable();
            });
        } else {
